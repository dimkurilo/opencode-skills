...
### Added
- **wave-spec — context placement prefix/tail + re-dispatch delta (CAS-173)**:
  - `wave-spec/SKILL.md` — секция §6e Context placement: stable prefix = ссылки на policy/schema/worker-contract/model-card/Prohibited (НЕ копии — источники авторитетны); task tail = base packet reference (`BASE_PACKET_PATH`) + current delta + exact evidence + iteration delta; retry = delta-only поверх immutable base (начальный packet + контракты авторитетны, не пересылаются и не заменяются). Precedence: (а) injected preamble — runtime identity (ROLE/SCOPE/MODE/COORDINATOR_HANDLE/TASK_ID/DISPATCH_ID), ВСЕГДА свежий через `dispatch --inject` (на retry TASK_ID/DISPATCH_ID меняются — preamble нельзя выкинуть); (б) immutable base brief + sources (SPEC/PLAN/worker-contract/model-card); (в) retry delta — только evidence/hypothesis/diff/next action. Delta НЕ может менять scope / write allowlist / acceptance / output contract / Prohibited (material change → reset/re-plan CAS-168, НЕ delta). Base недоступен → escalation, не угадывание. Дельта-строка NEXT_SESSION (`Changes vs I{N-1}`, §6c step 9) — обязательный паттерн и для worker re-dispatch: ретрай воркера ссылается на retry-delta path, не на полный предыдущий brief.
  - `worker-brief.md.tmpl` — поля `BASE_PACKET_PATH` / `ATTEMPT` / `RETRY_DELTA_PATH|none` + блок «Retry delta only» (initial dispatch delta = none; retry = только delta + path на base, base НЕ копируется в task text).
  - НОВЫЙ `assets/templates/re-dispatch-brief.md.tmpl` — delta-only retry packet (11 полей): `BASE_PACKET_PATH` (immutable base, обязателен; недоступен → escalation), `ATTEMPT`, `FAILURE_EVENT_PATH`, `FINGERPRINT`, `HYPOTHESIS`, `COUNT`, `RESET_REASON`, `PREVIOUS_EVIDENCE` (command + observed), `CHANGED_DIFF_REF`, `NEXT_ACTION`, `REVIEW_MODE` (если применимо). В задачу передаётся только tail + path на base; delta не может override base scope/acceptance/output contract; `COUNT` не self-increment (ledger — orchestrator).
  - `multi-model-orchestration/references/failure-handling.md` — «Recovery After Worker Crash»: concat `brief + 'Previous attempt failed'` заменён на re-dispatch packet flow (delta-only tail + `BASE_PACKET_PATH`); same-worker/re-plan (5th-fail) и writer-swap (API Retry Storm) сохранены; timeout/liveness/API-retry/tool-loop ветки не тронуты; retry доставляется только через `dispatch --inject`, не terminal send.
  - `multi-model-orchestration/references/worker-contract.md` — cross-reference: injected preamble всегда свежий; base authoritative; delta не override; `worker_done` на retry — новые task/dispatch ID + явный `--to`.
  - `NEXT_SESSION_ITER.md.tmpl` — комментарий retry-delta у `Changes vs {{prev_iteration}}`: worker re-dispatch ссылается на retry-delta path.
  - README.md / README.ru.md синхронизированы (**Templates (15) → (16)** — новый `re-dispatch-brief.md.tmpl`; добавлен параграф Context placement в How it works / Как работает).
- **wave-spec — scripted review packet updater (CAS-172)**:
  - НОВЫЙ `scripts/update_review_packet.py`: CLI `PACKET --updates-json UPDATES` + `--create` из замороженного `review-synthesis.md.tmpl` (CAS-171 template freeze — шаблон не менялся); JSON-операции только `string` / `{"append": ...}` / `{"file": ...}` на 8 allowlisted section IDs (`review_mode`, `verdict`, `findings`, `complementary_lenses`, `contradiction_resolution`, `closure_list`, `owner_decision`, `residual`); константные секции (Fix-round contract, Follow-up review) не обновляемы; fence-safe парсер (заголовки только вне fenced code; закрытие тем же типом и ровно той же длиной); exact canonical-value валидация (`review_mode` ∈ Mechanical|Simple|Ordinary|Strong; `verdict` ∈ APPROVED|NEEDS_CHANGES|BLOCKED) + ревалидация существующих секций при любом update; атомарность: парсинг+валидация в памяти → `mkstemp(dir=packet.parent)` → UTF-8 write → `flush`/`fsync` → `os.replace` (при ошибке исходник byte-identical); strict UTF-8 (включая file-операции); legacy packet refusal (APPROVE/APPROVE_WITH_CHANGES/CHANGES_REQUESTED); no LLM/no subprocess; ошибки section/line + тип нарушения без payload (secrets); unknown/foreign секции сохраняются byte-for-byte.
  - НОВЫЙ `scripts/test_update_review_packet.py` — 33 unittest по контракту (create/update/append/file; foreign byte-for-byte; fence safety; missing/duplicate/malformed; empty body; invalid mode/verdict; atomic failure; UTF-8; legacy refusal; no-LLM/no-subprocess; CLI contract).
  - `SKILL.md` — подсекция «Scripted packet update (review synthesis)»: workflow worker findings → coordinator decisions (JSON updates payload) → script update → read/verify packet → In Review; скрипт — детерминированный helper (не выбирает mode/verdict, не закрывает MAJOR, не заменяет approve/LAUNCH/In Review gates).
  - README.md / README.ru.md синхронизированы (15 templates — без изменений; новых шаблонов нет).
- **wave-spec + multi-model-orchestration — proportional review modes (CAS-171)**:
  - `wave-spec/SKILL.md` §Review modes (single source of truth для non-fidelity review depth): 4 режима — `Mechanical` (0 reviewers), `Simple` (1 cross-family low-effort), `Ordinary` (1 cross-family, default Qwen 3.8 Max), `Strong` (2 complementary lines: gpt-5.6-luna (OpenAI GPT-5.6; launch `codex --model gpt-5.6-luna -c model_reasoning_effort="max"`; «Luna Max» — shorthand, НЕ имя модели; `max` = reasoning effort) behavioral/security/fidelity lens + Qwen 3.8 Max static/architecture lens; swap to GLM 5.2 когда writer = Alibaba). **Owner-fix (2026-08-07):** (1) Терминология: «Luna Max» — НЕ имя модели; модель = `gpt-5.6-luna`, `max` = reasoning effort. Все формулировки исправлены: первое упоминание в каждом файле — полный launch-контракт, далее — `gpt-5.6-luna`; «Luna Max» допустим только как явный shorthand с пояснением «НЕ имя модели». (2) Strong-пара GPT-5.5+GLM 5.2 → gpt-5.6-luna + Qwen 3.8 Max (адаптация OFK «Sol High + Kimi K3»; Kimi не зарегистрирован в нашем стеке). (3) GPT-5.5 → опциональный security-гейт (историческая роль сохранена, NOT часть Strong-пары по умолчанию; 25× дороже luna). (4) `gpt-5.6-terra` / `gpt-5.6-sol` убраны из model-card (не планируем использовать — единственная OpenAI-модель = gpt-5.6-luna). (5) Цены компактно (одна строка на модель) + бенчи ПО effort-уровням. gpt-5.6-luna: AAII max 51 (none≈26.6/low≈30-33/medium 38/high 46/xhigh 49/max 51), $0.20/$1.20 (−80% с 30.07.2026). GPT-5.5: AAII xhigh 55 (high 53/medium 50/low 43/none 35), $5/$30, optional security gate, effort high практичный дефолт / xhigh для критичных. (6) GPT-5.6 бриф-формат (Goal → Success → Context → Constraints → Autonomy → Output → Stop) добавлен в multi-model SKILL.md §4 + routing.md (новая секция `### GPT-5.6 (gpt-5.6-luna, codex)`); гайд `guides/kak-pisat-zaprosy-dlya-gpt-5.6.md` (lean contracts; НЕ DeepSeek 【】; effort/verbosity — API). Precedence = floor + stricter-of (НЕ first-applicable-wins): user/repo review requirement = floor (минимальный режим; repo-floor = project-level review policy в AGENTS.md/SPEC, НЕ lifecycle/dispatch gates — они соблюдаются при любом режиме и floor не поднимают); risk triggers (security/RLS/auth/secrets/permissions, prod deploy/config/persistent data/schema migration, fidelity port/behavioral parity/reference→platform, abort/cost/state-transition semantics, unknown blast radius) → `Strong` unconditionally даже при низком floor; no runtime/control-flow/contract impact + checks green → `Mechanical`; low-risk → `Simple`; иначе `Ordinary`. Severity order для "stricter": Strong > Ordinary > Simple > Mechanical. Cross-family (`writer.family ≠ reviewer.family`) для ВСЕХ режимов с reviewer; `Mechanical` — единственный без reviewer (исключение Simple-without-cross-family отменено — low-risk работа получала бы ровно то слепое пятно, которое cross-family должен ловить). Lens assignment fixed by category (НЕ by preference): security/RLS/auth/secrets/permissions/fidelity/abort-cost → gpt-5.6-luna mandatory на behavioral lens; static/architecture → Qwen 3.8 Max (default) или GLM 5.2 (когда writer Alibaba). "No retries" (Strong) = не re-dispatch ревьюера внутри того же gate; НЕ отменяет fix-round контракт (max 2, round 3 escalation). Follow-up review только по user request или unresolved BLOCKER/HIGH (новый gate со своим mode selection). Solo-default НЕ отменяет selected `review_mode` (solo-written security change всё равно Strong).
  - STATUS `## Review gate` блок (orchestrator-owned, отдельный от 10-колоночного ledger CAS-167 и от canonical Handoff 6 headings — компактный snapshot может появиться только в `### Residual risk`): поля `review_mode` (Mechanical|Simple|Ordinary|Strong) / `selection_basis` (ALL applicable inputs, не первый сработавший — напр. `repo-floor=Ordinary + risk=prod → stricter=Strong`) / `review_dispatches` (0|1|2, per-dispatch reviewer name+family) / `strong_session_used` (true|false|n/a — atomic compare-and-set ДО первого Strong dispatch: false → atomically set true → dispatch 2 complementary; true → BLOCK + `blocked_by: strong_session_used` в STATUS `## Review gate` блок + owner/escalation outcome, НЕ тихий пропуск и НЕ второй Strong dispatch; флаг НЕ сбрасывается внутри сессии; transport/API failure на первом Strong dispatch = non-counting operational failure — без ledger increment и без автоматического второго Strong) / `follow_up_reason` (none|user-requested|unresolved-blocker-high) / `synthesis_path`. Workers сообщают reviewer findings через стандартный output contract; они НЕ пишут этот блок.
  - `review-synthesis.md.tmpl`: generic 0/1/2 reviewer inputs (Mechanical skip с возможностью не создавать артефакт; Simple/Ordinary single-reviewer findings table; Strong complementary lenses table + contradiction resolution со stricter-wins). Verdict enum `APPROVED | NEEDS_CHANGES | BLOCKED` (APPROVED — no material findings; NEEDS_CHANGES — any material finding включая LOW когда material; BLOCKED — security/fidelity/behavioral MAJOR от gpt-5.6-luna (behavioral) lens или finding небезопасное для deferral). Follow-up review только user request / unresolved BLOCKER-HIGH. Fix-round контракт (max 2, round 3 escalation) сохранён — **template freeze для CAS-172**.
  - `multi-model-orchestration/SKILL.md`: §3 step 8c — ветвление по `review_mode` (Mechanical → SKIP steps c–e + skip evidence в STATUS `## Review gate` блок и handoff `### Residual risk`; Simple → 1 cross-family low-effort reviewer; Ordinary → 1 default Qwen 3.8 Max; Strong → atomic compare-and-set `strong_session_used` BEFORE first dispatch + 2 complementary lines с cross-family для обеих). §4 — `REVIEW_MODE` поле в brief (orchestrator-set; reviewer НЕ вычисляет постфактум — mismatch с orchestration packet → BLOCKER, не silent reclassification; reviewer знает effort/lens context: Simple = model-specific low-effort launch contract, Strong = которая complementary lens). `references/routing.md` — §Review mode branching (таблица 4 режимов) + §Simple effort (model-specific launch contract: GLM 5.2/OpenCode Qwen 3.8 Max → `/variants low` + sleep 3; Qwen Code → `/effort medium` — минимальный поддерживаемый, `low` не поддерживается; GPT-5.5 codex CLI → `model_reasoning_effort="low"`). Solo-default НЕ отменяет selected `review_mode`.
  - `NEXT_SESSION_ITER.md.tmpl`: шаг 5 — Review по `{{review_mode}}` (mode branch; оркестратор выставляет `review_mode` в brief/STATUS `## Review gate` блок ДО первого dispatch — ревьюер не вычисляет режим постфактум).
  - README.md / README.ru.md синхронизированы (15 templates — без изменений; новых шаблонов нет).
...
### Fixed
- **wave-spec** — P1-2 из ревью 03 (secret redaction): value-aware grep-паттерн заменил наивный `'token|password|api[_-]?key|Bearer|secret'`. На типичной волне [transfer] (9 NEXT_SESSION_T0x, постоянная работа с `SOURCE_API_KEY`/`DEST_API_KEY`) старый паттерн давал 18 false positive (имена переменных в bash-скриптах) и 0 реальных утечек — оператор привыкал игнорировать. Новый паттерн: явные case-варианты ключевых слов (без `-i`, иначе `[a-z]` матчит UPPER) + требование lowercase в значении (отсекает `VAR_NAME`-формы) + значение 15+ chars. Тест на синтетике: 5/5 реальных секретов поймано, 2 UPPER-имени пропущено. На реальной волне: 0 FP (vs 18 у старого). Применено в `SKILL.md` §Wave closeout checklist + `quick-spec.md.tmpl`. Дополнительно: `worker-brief.md.tmpl` Forbidden — явно запрещает вставку secret VALUES в SUMMARY/EVIDENCE/CHANGES (variable NAMES ок; `[REDACTED]` если нужно для контекста).
