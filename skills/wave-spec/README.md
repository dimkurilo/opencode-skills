...
- **LAUNCH.md auto-generation (step 6b):** worker launch commands from model-card.md + PLAN.xml roles, cross-family check (writer.family ≠ reviewer.family), per-model review brief templates (all 6 models from model-card.md), prohibitions section, versioning on role change
- **NEXT_SESSION pattern (step 6c):** unique files `NEXT_SESSION_I{N}.md` per iteration (never overwrite), copy-paste block with full path (format by orchestrator model), root pointer `NEXT_SESSION.md`, 9 steps with verification gates (no SKILL.md duplication)
