...
- **Автогенерация LAUNCH.md (шаг 6b):** команды запуска воркеров из model-card.md + PLAN.xml roles, cross-family check (writer.family ≠ reviewer.family), шаблоны review brief по моделям (DeepSeek, GLM, Codex/GPT, Qwen Code), секция запретов, версионирование при смене ролей
