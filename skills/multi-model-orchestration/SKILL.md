...
| Security / RLS / auth / Storage review | **gpt-5.6-luna** (launch: `codex --model gpt-5.6-luna -c model_reasoning_effort="max"`; «Luna Max» — shorthand, НЕ имя модели) + DeepSeek V4 Flash (parallel) | OpenAI + DeepSeek | Gate + implementer; cross-family required (GPT-5.5 = optional opt-in strict gate для критичных) |
...
| **Fidelity port** (generator, vectorizer, reference→platform) | **DeepSeek V4 Flash** write (default) / **GLM 5.2** (multi-file) · **gpt-5.6-luna** ∥ **Qwen 3.8 Max** review | DeepSeek/Zhipu · OpenAI ∥ Alibaba | Evidence (production waves): Flash bulk implement; GLM exhaustive static parity; gpt-5.6-luna behavioral regression lens; Qwen architect/analyst. Dual review mandatory — complementary, not redundant |
...
- **NEW review_mode field**: every brief carries `REVIEW_MODE` set by the orchestrator from the precedence in `skills/wave-spec/SKILL.md` §Review modes. The reviewer uses it to know its effort/lens context (Simple = model-specific low-effort launch contract; Strong = which complementary lens — gpt-5.6-luna behavioral or Qwen 3.8 Max static). The reviewer does NOT re-classify the mode; mismatch with the orchestration packet → BLOCKER, not silent reclassification.
...
- **GPT-5.6 (gpt-5.6-luna / codex)**: Goal → Success → Context → Constraints → Autonomy → Output → Stop. Lean contracts: одна инструкция — один раз; НЕ использовать DeepSeek `【】`-инъекцию и «think step by step» (не control surface для GPT-5.6); effort/verbosity — API-параметр (launch contract `-c model_reasoning_effort=...`, не текст); Success = acceptance + validation; Autonomy — компактная policy один раз (research→report; fix→local+tests; confirm external/destructive/scope). Плотность: простая = Goal + Where + Success; средняя = + Context + Constraints + Autonomy; сложная = все 7 блоков. Гайд: `guides/kak-pisat-zaprosy-dlya-gpt-5.6.md`.
