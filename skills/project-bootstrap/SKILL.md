...
description: >
  ...
  по стандарту Agent Playbook. plan.md — стратегический план для человека (без статусов); SESSION_HANDOFF.md — операционное состояние; MEMORY.md — долговременная память.
...
**`plan.md`** — шаблон `assets/templates/plan.md.tmpl`. Стратегический план проекта для человека: карта фаз, описание каждой фазы (цель/вход/действия/выход/кто), правила, решения, блокеры. **БЕЗ статусов выполнения** — статусы в SESSION_HANDOFF.md.
...
**`SESSION_HANDOFF.md`** — шаблон `assets/templates/SESSION_HANDOFF.md.tmpl`. Только операционное состояние: CURRENT_FOCUS, TASK_BACKLOG, ENVIRONMENT_NOTES, FILE:LINE_ANCHORS. CONFIRMED_FACTS, UNRESOLVED_ISSUES, FAILED_APPROACHES — в MEMORY.md (долговременная память).
...
Шаблон `assets/templates/MEMORY.md.tmpl`. Единственный источник долговременной памяти. Секции:
- **CONFIRMED_FACTS** — каждый факт с источником (append-only)
- **UNRESOLVED_ISSUES** — проблемы без первопричины, severity: low/medium/high/critical
- **FAILED_APPROACHES** — что пробовали и почему не сработало
- **Решения** — ключевые архитектурные и стратегические решения
